...
def launch_gui(revenue_data):
    ...
    def show_histogram(frame):
        ...
        # Set the number of bins and determine the bin range based on revenue
        num_bins = 10
        ...
        for revenue in revenues:
            bin_index = int((revenue - min_revenue) / bin_size)
            if bin_index == num_bins:  # Handle edge case where revenue is exactly max_revenue
                bin_index -= 1
            bins[bin_index] += 1
        ...
        ax.set_title("Total Transaction Values (Histogram)", fontsize=12)
